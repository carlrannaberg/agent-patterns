...
export class SequentialProcessingService {
  ...
  async generateMarketingCopy(input: string) {
    this.logger.verbose(`Starting sequential processing for input: "${input.substring(0, 50)}..."`);
    ...
    this.logger.verbose(`Step 1 completed. Generated copy length: ${copy.length} characters`);
    ...
    this.logger.verbose(`Step 2 completed. Quality metrics: ${JSON.stringify(qualityMetrics)}`);
    ...
    if (
      ...
      this.logger.verbose('Step 3: Copy needs improvement, generating improved version');
      ...
      this.logger.verbose('Step 3: Copy quality is sufficient, no improvement needed');
    }
